...
Skýrslur {id=skyrslur data-icon="fa-notes-medical"}
=======================================================================
...
#Calendar dagsetningar:
dateInput('dagsetning_skyrslu', 'Dagsetning:',
          value = Sys.Date(), min = min(historical_data$date), max = date_data)
helpText('Birtist sem blá lína á grafi')
hr()
#Legutímar
helpText("Legutímar í prior")
radioButtons('los_val_skyrsla', 'Legutími', choices = c('covid.hi.is', 'LSH gögn', 'Fyrsta bylgja Covid-19'), selected = 'LSH gögn')

hr()
radioButtons('spa_choose', 'Spá', choices = c('covid.hi.is', 'Eigin spá', 'Úr skrá'), selected = 'Úr skrá')
observeEvent(input$bua_til_manual, {
  updateRadioButtons(session, "spa_choose", selected = 'Eigin spá')
})
```
[Búa til eigin spá](#manual_spa)
```{r}
hr()
helpText('Það tekur um 2 mínútur að birta skýrsluna')
actionButton("birta_skyrslu", "Birta skýrslu")
observeEvent(input$birta_skyrslu, {
  showNotification("Keyrir hermun...", id = 'msg_keyra_hermun1', type = 'message', duration = NULL)
})

# checkboxInput('show_history', "Sýna öll söguleg gögn", value = FALSE)
dateInput('filter_history', 'Sýna söguleg  gögn frá:', value = Sys.Date()-40, min = min(historical_data$date), max=max(historical_data$date))
numericInput('number_predicted_days', "Fjöldi spádaga", value = 14, min=1, max=80, step=1)

###### auka #####
hr()
#actionButton("saekja_skyrslu_data", "Sækja spá")
actionButton("saekja_skyrslu", "Sækja skýrslu")
actionButton("saekja_skyrslu_data_hospital", "Sækja spítalaspá")
```
...
```{r keyrsla skýrslu}
# renderText("Svörtu punktarnir eru söguleg gögn, bláa línan er dagurinn í dag og hinar línurnar eru spá fyrir valinn fjölda daga.")
...
prediction_type <- reactive({
  if(input$spa_choose=='covid.hi.is'){
    'hi'
  }
  else if(input$spa_choose=='Eigin spá'){
    'manual'
  }
  else if(input$spa_choose=='Úr skrá'){
    'from_file'
  }
  else
    stop("Unexpected dataset")
...
output_best_unfiltered <- eventReactive(input$birta_skyrslu, {
...
# historical_data_filtered <- eventReactive(input$birta_skyrslu, {
...
# observeEvent(input$saekja_skyrslu_data, {
#   write.table(select(output_best_unfiltered(), -count_historical, -best_alpha), file = paste0("report/", input$dagsetning_skyrslu, "_report_data.csv"), quote = F,row.names=F,sep=',')
# })

observeEvent(input$saekja_skyrslu, {
  write.table(select(output_best_unfiltered(), -count_historical, -best_alpha), file = paste0("report/", Sys.Date(), "_report_data.csv"), quote = F,row.names=F,sep=',')
  
  rmarkdown::render("report/report_new.Rmd", "html_document")
  openHTML(paste0("report/report_new.html"))
})

observeEvent(input$saekja_skyrslu_data_hospital, {
  write.table(select(output_best_unfiltered() %>% 
    filter(state!="home") %>% 
    arrange(date) %>%
    mutate(state=ifelse(state=="inpatient_ward", "Legudeild", "Gjörgæsla")) %>%
    mutate(median=round(median), lower=round(lower), upper=round(upper)), -count_historical, -best_alpha), 
    file = paste0("output_lsh/", input$dagsetning_skyrslu, "_covid_simulation_summary_hospital.csv"), quote = F,row.names=F,sep=',')
})
```
...
--------------------------------------
```{r}
renderText(paste("alpha =", output_best()$best_alpha[1]))
```
...
renderPlotly({
  ...
  # if(input$show_history==TRUE){
  #   p <- p + geom_point(data=filter(historical_data, state=="home"), aes(y=count, label="Söguleg gögn"), alpha=0.4)
  # }
...
renderPlotly({
  ...
  # if(input$show_history==TRUE){
  #   p <- p + geom_point(data=filter(historical_data, state=="inpatient_ward"), aes(y=count, label="Söguleg gögn"), alpha=0.4)
  # }
  ggplotly(p, tooltip = list('linetype', 'date', 'value', 'label')) #%>%
  #layout(legend = list(orientation = "h", x = 0, y =1))
})
...
renderPlotly({
  ...
  p <- p + geom_vline(xintercept = as.numeric(dagsetning_skyrslu_reactive()$dagsetning), linetype="dotted", color="dodgerblue")
  # if(input$show_history==TRUE){
  #   p <- p + geom_point(data=filter(historical_data, state=="intensive_care_unit"), aes(y=count, label="Söguleg gögn"), alpha=0.4)
  # }
  ggplotly(p, tooltip = list('linetype', 'date', 'value', 'label')) #%>%
  #layout(legend = list(orientation = "h", x = 0, y =1))
})
...
Stöðuupplýsingar {data-icon="fa-user-md"}
=======================================================================
...
```{r stillingar visual}
#Slider dagsetningar:
# sliderInput('dagsetning', 'Dagsetning:', min = min(historical_data$date), max = max(historical_data$date),
...
selectInput('choose_visual_graph', "Veldu upplýsingar til að sjá á grafi", choices = c("Dreifing á tíma frá upphafi einkenna", "Aldursdreifing", "Dreifing á fjölda undirliggjandi sjúkdóma"), multiple = FALSE)
```
...
graphInputHome <- reactive({
   switch(input$choose_visual_graph,
          "Dreifing á tíma frá upphafi einkenna" = plot_symptoms_start_home(),
          "Aldursdreifing" = plot_age_home(),
          "Dreifing á fjölda undirliggjandi sjúkdóma" = plot_num_comorbidity_home()
          )
  })

graphInputinpatient_ward <- reactive({
   switch(input$choose_visual_graph,
          "Dreifing á tíma frá upphafi einkenna" = plot_symptoms_start_iw(),
          "Aldursdreifing" = plot_age_iw(),
          "Dreifing á fjölda undirliggjandi sjúkdóma" = plot_num_comorbidity_iw()
          )
  })

graphInputIcu <- reactive({
   switch(input$choose_visual_graph,
          "Dreifing á tíma frá upphafi einkenna" = plot_symptoms_start_icu(),
          "Aldursdreifing" = plot_age_icu(),
          "Dreifing á fjölda undirliggjandi sjúkdóma" = plot_num_comorbidity_icu()
          )
  })
...
Flæði sjúklinga í faraldri {data-icon="fa-ambulance"}
=============================================
...
dateRangeInput('dagsetning_flow', 'Tímabil:', 
               start = ymd("2020-03-10"), end=Sys.Date()+1, min = min(historical_data$date), max = Sys.Date() + 30)
...
##### Auka #####
actionButton("saekja_fleadi_data", "Sækja gögn")
```
...
##### Sækja spá #####
observeEvent(input$saekja_fleadi_data, {
  write.table(transitions_simple(), file = "~/Downloads/transitions_simple.csv", quote = F,row.names=F,sep=',')
  write.table(transitions_complex(), file = "~/Downloads/transitions_complex.csv", quote = F,row.names=F,sep=',')
})
##########
...
#radioButtons('val_stikar_logistic', 'Stikar:', choices = c('Einfaldir', 'Flóknir'), selected = 'Einfaldir')
...
```{r download button logistic}
# Create placeholder for the downloadButton
uiOutput("downloadUI_logistic")

# Create the actual downloadButton
output$downloadUI_logistic <- renderUI({
  tagList(
    downloadButton("downBtn_logistic", "Sækja ný smit"),
  )
})

# Add download handling
output$downBtn_logistic <- downloadHandler(
  filename = function() {
    paste0(Sys.Date(), "_1_scenario_infected.csv")
  },
  content = function(file) {
    write.csv(smit_folk_logis(), 
...
Sviðsmyndir SIR {id=svidsmyndir data-icon="fa-chart-bar"}
===============================================

Stillingar {.sidebar}
---------------------------------------------------
```{r stillingar scenario}
renderText("Smella þarf á \"Keyra\" eftir að stilling er valin")
# Fyrir plottun
# Aldursdreifing
hr()
radioButtons('sir_aldur_val', 'Aldursdreifing', choices = c('Faraldurs', 'Þjóðar', 'Skv. stillingu:'))
conditionalPanel(
  "input.sir_aldur_val == 'Skv. stillingu:'",
  sliderInput("sir_aldur_dreif1",
              HTML('Alpha'),
              min = 0, max = 10, step=0.1, value = c(5)),
  sliderInput("sir_aldur_dreif2",
              HTML('Beta'),
              min = 0, max = 10, step=0.1, value = c(5))
)
hr()
...
sliderInput("sir_r_eff", 'Virkur smitstuðull (R)',
            min = 0.1, max = 3, step = 0.1, value = 0.7)
sliderInput("sir_dagar", 'Fjöldi daga til að teikna',
            min = 5, max = 75, step = 5, value = 30)
#Legutími
hr()
radioButtons('los_val', 'Legutími', choices = c('covid.hi.is', 'LSH gögn', 'Skv. stillingu:'))
conditionalPanel(
  "input.los_val == 'Skv. stillingu:'",
  sliderInput("los_vd",
              HTML('Veikindadagar'),
              min = 12, max = 22, step=1, value = c(17), ticks = F),
  sliderInput("los_ld",
...
renderText({
    paste0('Veikindadagar: ', los_dist()$vd, ' , ')
})
renderText({
    paste0('Dagar á legudeild: ', los_dist()$ld, ' , ')
})
renderText({
    paste0('Dagar á gjörgæslu: ', los_dist()$gd, ' , ')
})
renderText({
    paste0('Dagar heima fyrir legudeild: ', los_dist()$pre_lega, ' , ')
})
renderText({
    paste0('Dagar á legudeild fyrir gjörgæslu: ', los_dist()$pre_icu)
})

#Dagar á legudeild þurfa að vera færri en veikindadagar
observeEvent(input$los_vd,{
    updateSliderInput(session, "los_ld", max = input$los_vd-1)
})
#Dagar á gjörgæslu þurfa að vera færri en dagar á legudeild
observeEvent(input$los_ld,{
    updateSliderInput(session, "los_gd", max = input$los_ld-1)
})
#####
```

```{r aldursdreifing og LOS}
#### Age distributions
aldur_faraldur <- get_patient_transitions_at_date('base',date_observed = date_data) %>%
    distinct(patient_id) %>%
    inner_join(individs_splitting_variables,by='patient_id') %>%
    rename(splitting_variable=!!'age_official') %>%
    group_by(splitting_variable) %>%
...
aldur_hagstofa <- read_delim("dashboard/aldur_hagstofa.csv", ";", escape_double = FALSE, 
                             locale = locale(encoding = "ISO-8859-2"), trim_ws = TRUE, col_types = cols())

aldur_hagstofa <- aldur_hagstofa %>% mutate(Aldur=parse_number(Aldur))

aldur_hagstofa$Aldur <- seq(0, max(aldur_hagstofa$Aldur))
...
aldur_stilling <- reactive({
  N <- 100000
  x <- rbeta(N, input$sir_aldur_dreif1, input$sir_aldur_dreif2)*100
  dreifing <- data.frame(aldur=round(x))
  dreifing <- dreifing %>% mutate(splitting_variable=cut(dreifing$aldur, breaks = c(seq(0, 80, by = 10), Inf), 
                                             labels = aldur_faraldur$splitting_variable, right = FALSE))
  ...
  aldur_stilling <- left_join(select(aldur_faraldur, splitting_variable), dreifing) %>%
    mutate(prop=if_else(is.na(prop), 0, prop))
...
#### LOS ####
los_covid_hi_is <- data.frame("vd"=21, "ld"=14, "gd"=10, "pre_lega"=7, "pre_icu"=3)

los_lsh <- data.frame("vd"=14, "ld"=11, "gd"=9, "pre_lega"=3, "pre_icu"=1)

los_stilling <- reactive({
  data.frame("vd"=input$los_vd, "ld"=input$los_ld, "gd"=input$los_gd, "pre_lega"=input$los_vd-input$los_ld, "pre_icu"=input$los_ld-input$los_gd)
})
#####
```

```{r aldur og LOS reactive}
age_dist <- reactive({
  if (input$sir_aldur_val == 'Þjóðar')
    aldur_hagstofa
  else if (input$sir_aldur_val == 'Faraldurs')
   aldur_faraldur
  else if (input$sir_aldur_val == 'Skv. stillingu:')
    aldur_stilling()
  else
    stop("Unexpected dataset")
})
...
los_dist <- reactive({
    if (input$los_val == 'covid.hi.is')
    los_covid_hi_is 
  else if (input$los_val == 'LSH gögn')
   los_lsh
  else if (input$los_val == 'Skv. stillingu:')
    los_stilling()
  else
    stop("Unexpected dataset")
})

actionButton("sir_keyra", "Keyra")
```

```{r download button SIR}
# Create placeholder for the downloadButton
uiOutput("downloadUI")
...
output$downloadUI <- renderUI({
  tagList(
    downloadButton("downBtn1", "Sækja ný smit"),
  )
})
...
### Aldursdreifing
```{r age plot}
renderPlotly({plot_age_distribution(age_dist())})
```

### Ný smit
```{r ný smit data & ný smit plot}
...
renderPlotly({
p <- ggplot(smit_folk()) +
    geom_col(aes(x=dags, y=nysmit), fill="#3b6978") +
    theme_minimal() + 
    labs(y="Ný smit", x=NULL)
ggplotly(p)
})
...
ferguson_data <- run_ferguson_simulation_new(infected_distr, los_dist(), age_dist())
ferguson_data %>% 
          mutate(state=ifelse(state=="hospital", "inpatient_ward", state))
})
```
...
### Heimaeinangrun
```{r plot scenario heima}
renderPlotly({
p <- plot_ferguson_prediction_state("home", tot_dat(), color_palette[1])
ggplotly(p, tooltip = list('linetype', 'date', 'value'))
})
```

### Legudeild
```{r plot scenario lega}
renderPlotly({
p <- plot_ferguson_prediction_state("inpatient_ward", tot_dat(), color_palette[2])
ggplotly(p, tooltip = list('linetype', 'date', 'value'))
})
...
```{r plot scenario icu}
renderPlotly({
p <- plot_ferguson_prediction_state("intensive_care_unit", tot_dat(), color_palette[3])
ggplotly(p, tooltip = list('linetype', 'date', 'value'))
})
```

Eigin spá {id=manual_spa .hidden}
==========================
