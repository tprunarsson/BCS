...
radioButtons('spa_choose', 'Spá', choices = c('covid.hi.is', 'Eigin spá'), selected = 'covid.hi.is')
...
date_prediction <- reactive({
  if(input$spa_choose=='covid.hi.is'){
    validate(
       need(input$dagsetning_skyrslu>=find_most_recent_prediction_date(), "Engin spá er til á þessum stað fyrir þessa dagsetningu")
     )
     find_most_recent_prediction_date()
  }
  else if(input$spa_choose=='Eigin spá'){
    validate(
    ...
    validate(
      need(input$dagsetning_skyrslu>=ymd("2020-10-16"), "Engin spá er til á þessum stað fyrir þessa dagsetningu")
    )
    ymd("2020-10-16") 
  }
  else
...
--------------------------------------
```{r}
renderText(paste("Spá um fjölda greindra smita nær frá", as.Date(date_prediction())))
```
