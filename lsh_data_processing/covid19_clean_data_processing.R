...
path_to_data <- '~/projects/covid/BCS/lsh_data_elias/'
#path_to_data <- ''
...
data_covid_id_raw <- read_excel(file_path_covid_id, skip = 3)
covid_id <- rename(data_covid_id_raw, kt = `Kennitala/gervikennitala`, person_id = `Person Key`)
...
individs_extended <-select(data_pcr_raw, kt, sex, age, test_nr, date, result) %>%
...
hospital_visits_filtered <- select(data_lsh_raw, kt, mortality, date, hospital_day, icu_given, date_start_icu, date_end_icu, date_admission, date_discharge, admitted_from) %>%
...
individs_extended <- left_join(individs_extended, select(hospital_visits_filtered, kt, outcome, unit_in, date_in, in_hospital_before), by="kt") %>%
                      ...
                      # left_join(., select(data_lsh_raw, kt, risk), by="kt") %>%
...
interview_last_date <- data_phone_raw %>%
                       group_by(.,kt) %>%
...
date_discharge_phone <- data_phone_raw %>%
                        group_by(.,kt) %>%
                        filter(call_nr==min(call_nr)) %>%
                        filter(`diagnosis_clinical:Yes`==0) %>%
...
date_symptoms_phone <- data_phone_raw %>%
                       group_by(.,kt) %>%
                       filter(call_nr==min(call_nr)) %>%
                       filter(`diagnosis_clinical:Yes`==0) %>%
...
#Undirliggjandi sjukdomar ur data_phone_raw
comorbidities_tafla <- select(data_phone_raw,kt,call_nr,date,dm_i,dm_ii,cardiovascular_disease,hypertension,pulmonary_disease,chronic_kidney_disease,current_cancer) 
