...
dateInput('sir_start_date', 'Dagsetning fyrsta smits',
          value = Sys.Date() + 1, min = '2020-03-01', max = Sys.Date() + 30)
numericInput("sir_utsettir", 'Fjöldi útsettra',
            min = 10, max = 1000, value = 300)
radioButtons('sir_aldur_val', 'Aldursdreifing', choices = c('Þjóðar', 'Faraldurs', 'Skv. Stillingu:'))
sliderInput("sir_aldur_dreif", '0-50 -- +51',
            min = 0, max = 100, value = c(60))
p('Staða við greiningu (%)')
sliderInput("sir_stada", 'Heima -- Legudeild -- Gjörgæsla',
            min = 0, max = 100, value = c(40, 70))

ratio_heima <- reactive(as.numeric(input$sv_stada[1]))
ratio_legu <- reactive(as.numeric(input$sv_stada[2]) - as.numeric(input$sv_stada[1]))
ratio_gjor <- reactive(100 - as.numeric(input$sv_stada[2]))
renderText(paste0('Heima: ', ratio_heima(), '%'))
renderText(paste0('Legudeild: ', ratio_legu(), '%'))
renderText(paste0('Gjörgæsla: ', ratio_gjor(), '%'))
...
### SIR kúrfa
```{r fig.height=2}
# staerd <- sliderInput(sir_staerd, 'Útsettir', min = 10, max = 1000)
# observeEvent(input$sir_keyra, ({
  shinySIR::run_shiny(model = "SIR", tmax = 60, ics =c(S = 100, I = 1, R = 2), showtable = F)
# }))
```

sviðsmyndir-legudeild-gjörgæsla
-------------------------------
