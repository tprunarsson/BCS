...
############ Load csv data
data_length_of_stay <- load_git_data('length_of_stay_empirical', start_date = '2020-04-08') %>% 
    recode_states()

data_all_simulation <- load_git_data('covid_simulation', data_folder = 'simulation_history', start_date = '2020-04-09') %>% 
  dplyr::rename(sim_date = date) 
...
sliderInput('dagsetning_sk', 'Dagsetning:', min = start_date, max = max(data_all_simulation$sim_date),
            value = end_date, step = 1, timeFormat = "%d %b")

```


Skýrslutexti {data-height=30}
-----------------------------------------------------------
...
data_simulation_quantiles <- reactive({
  data_all_simulation %>% 
    filter(sim_date == input$dagsetning_sk) %>%
    mutate(date = sim_date + day) %>%
...
plot_simulation <- function(state) {
  ...
  ggplot(data = p_data, aes(x = date, y = value)) +
...
# # Göngudeild
# prop_visits <- load_git_data('prop_outpatient_clinic', 
#                              data_folder = 'outpatient_clinic_history', 
#                              start_date = '2020-04-08')
# 
# window_size <- 7
# # prop_visit_for_date <- reactive(
# #   prop_visits %>% 
# #     filter(date == input$dagsetning - window_size) %>% 
# #     select(prop_visits_last_week) %>% 
# #     as.numeric()
# # )
# 
# outpatient_clinic_sim_visits <- reactive({
#   prop_visit_for_date <- prop_visits %>% 
#     filter(date == input$dagsetning - window_size) %>% 
#     select(prop_visits_last_week) %>% 
#     as.numeric()
#   data_all_simulation %>%
...
```{r}
sliderInput('dagsetning', 'Dagsetning:', min = start_date, max = max(data_length_of_stay$date),
            value = end_date, step = 1, timeFormat = "%d %b")
...
renderPlot({
  ...
  if(input$festa_asa)
    p <- p + lims(x = c(0, 35), y = c(0, max(data_length_of_stay$count)))
  print(p)
})
```
