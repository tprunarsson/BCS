...
if()
dates_start <- seq(date_observed_start+1,date_data-1,by=1)
dates_expanded <- expand_grid(date_observed=seq(date_observed_start,date_data-2,by=1),date=seq(date_observed_start+1,date_data-1,by=1),state=factor(states_in_order,levels=states_in_order,labels=states_labels_in_order))
for(i in seq_along(dates_observed)){
    path_simulation_data <- paste0('../output/',dates_start[i],'_',id,'_',dates_observed[i],'_covid_simulation.csv')
...
simulation_summary <- right_join(simulation_summary,dates_expanded,by=c('dates_observed','date','state'))

pred_animation <- ggplot(data=mutate(simulation_summary)) +
    ...
    transition_time(date_observed)
animate(pred_animation, height = 800, width = 1600)
anim_save("~/Downloads/fixed_start_prediction.gif", animation = last_animation())
