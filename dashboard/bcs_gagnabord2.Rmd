...
dateInput('dagsetning_skyrslu', 'Dagsetning:',
          value = Sys.Date(), min = min(historical_data$date), max = Sys.Date() + 30)
helpText('Birtist sem blá lína á grafi')
hr()
#Legutímar
helpText("Legutímar í prior")
radioButtons('los_val_skyrsla', 'Legutími', choices = c('covid.hi.is', 'LSH gögn', 'Fyrsta bylgja Covid-19'))

hr()
radioButtons('spa_choose', 'Spá', choices = c('covid.hi.is', 'Eigin spá', 'Úr skrá'))
...
output_best_unfiltered <- eventReactive(input$birta_skyrslu, {
  ...
  best_weighted_function(input$dagsetning_skyrslu, los_setting_skyrsla(), prediction_type(), date_prediction())
})
...
observeEvent(input$saekja_skyrslu_data, {
  write.table(output_best_unfiltered(), file = paste0("~/Downloads/", input$dagsetning_skyrslu, ".csv"), quote = F,row.names=F,sep=',')
})
```
...
#Aldursdreifing
radioButtons('logistic_aldur_val', 'Aldursdreifing', choices = c('Fyrsta bylgja Covid-19', 'Önnur bylgja Covid-19', 'Þjóðar', 'Skv. stillingu:'))
conditionalPanel(
...
setwd("../lsh_data_processing")
source('age_dist_new_data.R')
aldur_faraldur2_logistic <- age_dist_new_data()
setwd("../dashboard")
...
age_dist_logistic <- reactive({
  ...
  else if (input$logistic_aldur_val == 'Fyrsta bylgja Covid-19')
   aldur_faraldur_logistic
  else if (input$logistic_aldur_val == 'Önnur bylgja Covid-19')
